...
function validateVelocity() {
  ...
  if (velocityInput.value < 1) {
    alert("Velocity must be greater than 0");
    velocityInput.value = "";
...
function calculate(angle, velocity) {
  ...
  document.getElementById('height').innerHTML = calcHeight(verticalVelocity, tMaxHeight).toFixed(3);
  document.getElementById('distance').innerHTML = calcDistance(horizontalVelocity, tLanding).toFixed(3);
}
